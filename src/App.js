...
class App extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevState.initialMonthsList !== this.state.initialMonthsList) {
      this.updateDaysWithDatesList()
    }
  }

  updateDaysWithDatesList() {
    const daysWithDatesList = this.createDaysWithDatesList()
    this.setState({daysWithDatesList})
  }

  createDaysWithDatesList = () => {
    const {initialMonthsList} = this.state
    const daysWithDatesList = initialMonthsList.map(month => ({
      ...month,
      days: daysList.map(day => ({
        ...day,
        dates: month.dates.filter(date => {
          const dateObj = new Date(`2024-01-${date.date}`)
          return (
            dateObj.getDay() === day.dayNumber ||
            (day.dayNumber === 0 && dateObj.getDay() === 7)
          )
        }),
      })),
    }))

    return daysWithDatesList
  }
  ...
  handleNextMonth = () => {
    const {selectedMonthIndex, initialMonthsList} = this.state
    const maxIndex = initialMonthsList.length - 1
    const newIndex = Math.min(selectedMonthIndex + 1, maxIndex)
    this.setState({selectedMonthIndex: newIndex})
  }
  ...
  // handleFilterEmojiChange = emojiName => {
  //   this.setState({filterEmoji: emojiName})
  // }
  ...
  calculateFilterCount = () => {
    const {selectedDay, activeEmoji, initialMonthsList, selectedMonthIndex} =
      this.state
    ...
    const selectedMonth = daysWithDatesList[selectedMonthIndex]

    if (!selectedMonth) {
      return 0
    }

    const selectedDayObject = selectedMonth.days.find(
    ...
    const filterCount = selectedDayObject.dates.reduce(
      (count, date) => count + (date.emojiName === activeEmoji ? 1 : 0),
      0,
    )
  ...
  handleDateClick = (monthIndex, dateIndex) => {
    const {activeEmoji, initialMonthsList} = this.state
    const updatedMonthsList = [...initialMonthsList]
    const selectedDate = updatedMonthsList[monthIndex].dates[dateIndex]

    selectedDate.emojiUrl =
      activeEmoji !== selectedDate.emojiName
        ? emojisList.find(emoji => emoji.emojiName === activeEmoji)?.emojiUrl ||
          ''
        : ''

    selectedDate.emojiName =
      activeEmoji !== selectedDate.emojiName ? activeEmoji : ''

    this.setState({initialMonthsList: updatedMonthsList})
  }
            ...
            render={() => (
              <LanguageContext.Provider
                value={{
                  ...
                  createDaysWithDatesList: this.createDaysWithDatesList,
                  handlePreviousMonth: this.handlePreviousMonth,
                  handleNextMonth: this.handleNextMonth,
                  handleDayChange: this.handleDayChange,
                  calculateFilterCount: this.calculateFilterCount,
